...
function mapToLearnItem(
  ...
  return {
    ...
    type: forcedType ? normalizeType(forcedType) : normalizeType(safeText(item.type)),
    level,
...
function scoreItem(item: MicrosoftCatalogRecord, keywords: string[], products: string[]) {
  const haystack = [
    safeText(item.title),
  ...
  const itemProducts = safeArray(item.products).join(" ").toLowerCase()
  for (const product of products) {
    if (itemProducts.includes(product.toLowerCase())) score += 2
  }

  if (safeText(item.title).toLowerCase().includes(keywords[0] || "")) {
    score += 2
  }
...
function dedupeItems(items: LearnItem[]) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    const courses = catalog.instructorLedTrainingCourses || []
    ...
    const targetCourse = courses.find((item) => {
      const itemSlug = slugFromMicrosoftUrlOrTitle(
        safeText(item.url),
        safeText(item.title)
      )
    ...
    const keywords = buildKeywordSet(
      title ||
        safeText(targetLearningPath?.title) ||
        safeText(targetModule?.title) ||
        safeText(targetCourse?.title),
      products
    )

    // learning path -> modules
    if (targetLearningPath || type === "learningPath") {
      const relatedModules = modules
        .map((item) => ({
          raw: item,
          score: scoreItem(item, keywords, products),
        }))
      ...
      return NextResponse.json(
        { items: dedupeItems(relatedModules) },
        { status: 200 }
      )
    }
    ...
    if (targetModule || type === "module") {
      ...
      return NextResponse.json(
        { items: dedupeItems(relatedUnits) },
        { status: 200 }
      )
    }

    // course -> related modules
    if (targetCourse || type === "course") {
      const relatedModules = modules
        .map((item) => ({
          raw: item,
          score: scoreItem(item, keywords, products),
