    ...
    <div className="group overflow-hidden rounded-[28px] border border-white/10 bg-gradient-to-b from-white/[0.05] to-white/[0.02] transition-all hover:-translate-y-1 hover:border-cyan-400/20 hover:shadow-[0_16px_60px_rgba(0,0,0,0.35)]">
      <div className="relative h-56 w-full overflow-hidden bg-[#08101f]">
        ...
        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
          ...
          {hasVideo && (
            <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/40 px-3 py-1 text-xs text-white backdrop-blur-md">
    ...
    return (
      <div className="rounded-3xl border border-white/10 bg-black/20 p-5 backdrop-blur-md">
        ...
        <p className="mt-2 text-sm text-zinc-400">
          Modules, learning paths, courses, certifications, and exams will show
          here with real content cards and preview media.
        </p>
      </div>
    )
  }
...
export default function LearnPage() {
  ...
  const continueItems = useMemo(() => items.slice(0, 3), [items])
  const quickItems = useMemo(() => items.slice(3, 7), [items])
    ...
    <div className="min-h-screen w-full bg-[#050816] text-white">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-6 md:px-6 xl:px-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <h1 className="max-w-3xl text-4xl font-semibold tracking-tight text-white md:text-5xl xl:text-6xl">
                  Learn with real content. Preview classes. Grow with structure.
                </h1>
                <p className="max-w-2xl text-sm leading-7 text-zinc-300 md:text-base">
                  Search professional learning content, preview real class media
                  when available, and keep your technical growth system inside one
                  premium dashboard.
                </p>
              </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            title="Preview Ready"
            value={String(items.filter((item) => item.videoPreviewUrl).length)}
            subtext="Items with video preview"
            icon={<Video className="h-5 w-5" />}
          />
          <StatCard
            title="Content Types"
            value={String(typeFilters.length - 1)}
            subtext="Modules, paths, courses, exams"
        ...
        <section className="grid gap-6 xl:grid-cols-2">
          <div className="space-y-5">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-400">
                Fast access to active learning content.
              </p>
            </div>
              ...
              {continueItems.length > 0 ? (
                continueItems.map((item) => (
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-zinc-400">
                          {item.provider} • {formatDuration(item.durationInMinutes)}
                        </p>
                      </div>
                    ...
                    <Link
                      href={`/dashboard/learn/course/${item.slug}`}
                      className="inline-flex items-center gap-2 rounded-2xl bg-cyan-400 px-4 py-2 text-sm font-medium text-black"
                    >
                      Open content
                      <ChevronRight className="h-4 w-4" />
                    </Link>
                ...
                <div className="rounded-[26px] border border-dashed border-white/10 bg-white/[0.02] p-8 text-center">
                  <BookOpen className="mx-auto mb-4 h-8 w-8 text-zinc-500" />
                  <h3 className="text-lg font-semibold text-white">
                    Nothing available yet
                  </h3>
                  <p className="mt-2 text-sm text-zinc-400">
                    Search content and your active learning cards will appear here.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-400">
                Premium quick open area for selected learning items.
              </p>
            </div>
              ...
              {quickItems.length > 0 ? (
                quickItems.map((item) => (
                  ...
                  >
                    ...
                    <Link
                      href={`/dashboard/learn/course/${item.slug}`}
                      className="shrink-0 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-zinc-200 transition hover:bg-white/[0.08]"
                    >
                      Open
                    </Link>
                  </div>
                ...
                <div className="rounded-[26px] border border-dashed border-white/10 bg-white/[0.02] p-8 text-center">
                  <FolderHeart className="mx-auto mb-4 h-8 w-8 text-zinc-500" />
                  <h3 className="text-lg font-semibold text-white">
                    No quick access items yet
                  </h3>
                  <p className="mt-2 text-sm text-zinc-400">
                    Search content and build your learning system here.
                  </p>
                </div>
              )}
            </div>
