...
function ScriptStudioVisual() {
  return (
    <div className="relative flex h-[340px] w-full items-center justify-center overflow-hidden rounded-[32px] border border-yellow-500/20 bg-[radial-gradient(circle_at_center,rgba(250,204,21,0.08),transparent_48%),linear-gradient(180deg,rgba(10,10,10,0.98),rgba(18,18,18,0.98))]">
      ...
      <div className="absolute h-[185px] w-[185px] rounded-full border border-yellow-300/20 animate-spin [animation-direction:reverse] [animation-duration:12s]" />

      <div className="absolute left-[10%] top-[18%] rounded-full border border-yellow-400/20 bg-yellow-500/10 px-4 py-2 text-xs text-yellow-200/90 shadow-[0_0_30px_rgba(250,204,21,0.12)] rotate-[-8deg]">
        hook
      </div>
      <div className="absolute right-[12%] top-[24%] rounded-full border border-amber-400/20 bg-amber-500/10 px-4 py-2 text-xs text-amber-200/90 shadow-[0_0_30px_rgba(250,204,21,0.12)] rotate-[8deg]">
        scene
      </div>
      <div className="absolute left-[12%] bottom-[20%] rounded-full border border-orange-400/20 bg-orange-500/10 px-4 py-2 text-xs text-orange-200/90 shadow-[0_0_30px_rgba(250,204,21,0.12)] rotate-[6deg]">
        flow
      </div>
      <div className="absolute right-[10%] bottom-[18%] rounded-full border border-yellow-300/20 bg-yellow-500/10 px-4 py-2 text-xs text-yellow-200/90 shadow-[0_0_30px_rgba(250,204,21,0.12)] rotate-[-6deg]">
        CTA
      </div>

      <div className="relative w-[78%] rounded-[28px] border border-yellow-300/20 bg-black/45 p-6 shadow-[0_0_50px_rgba(250,204,21,0.12)] backdrop-blur-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-zinc-300">
              Premium video script structure for short-form creator content.
            </p>
          </div>

          <div className="flex h-14 w-14 items-center justify-center rounded-full border border-yellow-300/30 bg-[radial-gradient(circle_at_30%_30%,rgba(255,240,180,0.95),rgba(250,204,21,0.45),rgba(120,80,10,0.15))] text-2xl shadow-[0_0_40px_rgba(250,204,21,0.28)]">
            🎬
          </div>
        </div>

        <div className="space-y-3">
          <div className="h-3 w-[84%] rounded-full bg-gradient-to-r from-yellow-300/80 via-yellow-500/30 to-transparent" />
          <div className="h-3 w-[62%] rounded-full bg-gradient-to-r from-yellow-200/70 via-yellow-400/25 to-transparent" />
          <div className="h-3 w-[92%] rounded-full bg-gradient-to-r from-amber-300/70 via-yellow-400/20 to-transparent" />
          <div className="h-3 w-[74%] rounded-full bg-gradient-to-r from-yellow-200/70 via-yellow-500/25 to-transparent" />
          <div className="h-3 w-[56%] rounded-full bg-gradient-to-r from-orange-300/60 via-yellow-400/20 to-transparent" />
          <div className="h-3 w-[80%] rounded-full bg-gradient-to-r from-yellow-300/70 via-yellow-400/25 to-transparent" />
        </div>
      </div>
    </div>
  )
...
export default function ScriptsPage() {
  ...
  async function handleGenerate() {
    try {
      ...
      setResult("Failed to generate scripts.")
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="grid gap-8 xl:grid-cols-[1.15fr_0.85fr] xl:items-center">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-base leading-7 text-zinc-400">
              Generate premium short-form scripts with better flow, stronger opening
              structure, and cleaner delivery for creator content.
            </p>

            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              <StatCard label="Mode" value="Premium" />
              <StatCard label="Output" value={`${resultCount}`} />
              <StatCard label="Focus" value="Story Flow" />
            </div>
          </div>
        ...
        <div className="rounded-[30px] border border-yellow-500/20 bg-zinc-950 p-6 shadow-[0_0_40px_rgba(250,204,21,0.05)]">
          <div className="mb-6">
            ...
            <h2 className="mt-2 text-2xl font-bold text-yellow-400">
              Build your script request
            </h2>
            <p className="mt-2 text-sm text-zinc-400">
              Choose the topic, platform, audience, and language to generate
              stronger, more cinematic short-form scripts.
            </p>
          </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <input
                ...
                placeholder="Discipline mindset"
            ...
            <div className="rounded-2xl border border-yellow-500/15 bg-yellow-500/5 p-4 text-sm text-zinc-300">
              This module is built for short-form video scripts, motivational
              storytelling, product scripts, creator scripts, and premium content flow.
            </div>
        ...
        <div className="rounded-[30px] border border-yellow-500/20 bg-zinc-950 p-6 shadow-[0_0_40px_rgba(250,204,21,0.05)]">
          ...
          <div className="mt-5 rounded-3xl border border-zinc-800 bg-black/30 p-5">
            <pre className="min-h-[420px] whitespace-pre-wrap text-sm leading-7 text-zinc-300">
              {result ||
                "Your scripts will appear here. Generate premium short-form scripts for your videos, motivational posts, creator content, product storytelling, and sales flow."}
            </pre>
          </div>
